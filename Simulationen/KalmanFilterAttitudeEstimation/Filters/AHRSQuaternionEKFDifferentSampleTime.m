classdef AHRSQuaternionEKFDifferentSampleTime < handle %matlab.System %
    ...
    % This filter is similar to the AHRSQuaternionEKF filter, but with the
    % difference, that when due to the sample times of the sensors a value
    % is not available, not the previous value will be used, but the
    % correctionstep for this sensor will be skipped
    ...
    methods
        function obj = AHRSQuaternionEKFDifferentSampleTime(varargin) %sampleTime, x0, P0, delta_i, delta_d, g_mps2, sigma_2_quaternion, sigma_2_a, sigma_2_b_w, sigma_2_yaw)               
        ...
        % estimate states
        function performEKF(obj, u, y, acc_available, mag_available)
            ...
            % accelerometer
            if acc_available
                [y_hat, H_lin] = obj.y_hat_and_H_lin(obj.EKF.x, u, 0);
                obj.EKF.R_k = 1/obj.sampleTime * obj.R(1:3, 1:3);
                y_part = y(1:3);
            ...
            % magnetometer
            if mag_available
                [y_hat, H_lin] = obj.y_hat_and_H_lin(obj.EKF.x, u, 1);
                obj.EKF.R_k = 1/obj.sampleTime * obj.R(4:6, 4:6);
                y_part = y(4:6);
                obj.EKF.correctorStep(y_part, y_hat, H_lin);
            end
        ...
        function [y_hat, H_lin] = y_hat_and_H_lin(obj, x, u, nr)
            ...
            if nr == 0
                ...
                % magnetometer
                y_hat = [cos((pi*delta_d)/180)*cos((pi*delta_i)/180)*(1-2*(q_3^2+q_2^2))-2*sin((pi*delta_i)/180)*(q_1*q_3-q_0*q_2)-2*sin((pi*delta_d)/180)*cos((pi*delta_i)/180)*(q_0*q_3+q_1*q_2);
