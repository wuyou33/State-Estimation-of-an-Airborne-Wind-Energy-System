classdef AHRSEulerEKFDifferentSampleTime < handle
    ...
    methods
        ...
        function obj = AHRSEulerEKFDifferentSampleTime(sampleTime, delta_i, delta_d, g_mps2, x0, P0, sigma_2_w, sigma_2_a, sigma_2_b_w, sigma_2_yaw) 
        ...
        function [y_hat, H_lin] = y_hat_and_H_lin(obj, x, u, nr)
            ...
            if nr == 0
                ...
                H_lin = [0,	sin((pi*delta_d)/180)*cos((pi*delta_i)/180)*sin(psi)*sin(theta)-cos((pi*delta_d)/180)*cos((pi*delta_i)/180)*cos(psi)*sin(theta)+sin((pi*delta_i)/180)*cos(theta),	-cos((pi*delta_d)/180)*cos((pi*delta_i)/180)*sin(psi)*cos(theta)-sin((pi*delta_d)/180)*cos((pi*delta_i)/180)*cos(psi)*cos(theta),	0,	0,	0;
